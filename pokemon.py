...
def login(num): 
    ...
    if counter == 1:
        ...
        print("\n")
        user = session.query(User).filter(User.trainer_number == num).first()
        print(f"LOGIN USER INFO: {user}")
        account(user)
    else:
...
def pokemon():
    poke = (session.query(Pokemon)[random.randint(0,20)])
...
def fight(pokemon1, pokemon2, user):
    print("\n")
